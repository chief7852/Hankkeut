...
<script type="text/javascript">
	...
	$(function() {
		$('#addBtn').on('click', function() {
			...
			Form.action = "<%=request.getContextPath()%>/free/freeUpdate.ddit";
			Form.submit();
		});
	});
</script>
...
<body>
<%
	freeBoardVO boardVo = (freeBoardVO) request.getAttribute("boardVo");
	
%>
<form id="noticeBoard" action="<%=request.getContextPath()%>/free/freeInsert.ddit">
	<input type="hidden" name="n_board_num" value="<%=boardVo.getF_board_no()%>">
			...
			<div class="board_write">
				<div class="title">
					<dl>
						<dt>제목</dt>
						<dd>
							<input type="text" name="n_board_title" value="<%=boardVo.getF_board_title()%>">
						</dd>
					</dl>
				</div>
				<hr>
				<div class="cont" style='font-size:30pt'>
					<textarea name="n_board_contents" style='font-size:30pt'><%=boardVo.getF_board_contents().replaceAll("<br>", "\n")%></textarea>
				</div>
			</div>
