...
<script type="text/javascript">
	...
	$(function() {
		$('.nav li').on('click', function() {
			...
			if(liAttr == 'passUpdateLi') {
				...
				$('#deleteUserForm').css('display', 'none');
				$('#hp').parents('.tab-content').find('.msg').empty();
	...
	$(function() {
		function nopro(input,text) {
	    	$(input).parents('.tab-content').find('.msg')
	                                 .html(text).css('color','red')
	        $(input).parents('.tab-content').find('.sp1').empty();                
	    }
		function okpro(input) {
		    vsp = $(input).parents('.tab-content').find('.sp1')
		    $(vsp).empty();
		...
		$('#hp').on('keyup', function() {
			...
			if (reghp.test(hpvalue)) {
				vsp = $(this).parents('.tab-content').find('telmsgsp');
			    $(vsp).empty();
			    ...
			    $(this).parents('.tab-content').find('.telmsg').html("")
			} else {
				$(this).parents('.tab-content').find('.telmsg').html("3자리-4자리-4자리로 입력해 주세요").css('color','red');
				$(this).parents('.tab-content').find('.telmsgsp').empty();
			}
		});
		$('#pwd').on('keyup', function(){
	         psvalue = $(this).val().trim();
	         passreg = /^.*(?=^.{8,12}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/;

	         if(passreg.test(psvalue)){
	        	 vsp = $(this).parents('.tab-content').find('.sp');
				    $(vsp).empty();
				    ...
				    $(this).parents('.tab-content').find('.msg').html("")
	         }else{
	         	$(this).parents('.tab-content').find('.msg').html("특수문자  문자  숫자 를 포함해서 8~12자리로 입력하세요").css('color','red');
				$(this).parents('.tab-content').find('.sp').empty();
	         }
		});
		$('#pwd2').on('keyup', function(){
	         ...
	         if(passreg.test(psvalue)){
	        	 vsp = $(this).parents('.tab-content').find('.sp2');
				    $(vsp).empty();
				    ...
				    $(this).parents('.tab-content').find('.msg2').html("")
	         }else{
	        	 $(this).parents('.tab-content').find('.msg2').html("특수문자  문자  숫자 를 포함해서 8~12자리로 입력하세요").css('color','red');
				 $(this).parents('.tab-content').find('.sp2').empty();
	         }
		});
			...
			<div class="tab-content col-sm-9">
				<form id="passUpdateForm" action="<%=request.getContextPath()%>/mypage/updateInfo.ddit">
					<div id="passUpdate" class="tab-pane fade in active">
						<!-- 비밀번호 수정 -->
						<img src='./img/boy_basic.png'><br>
						<label for="usr">현재 비밀번호</label>
						<input type="password" class="form-control pw" name="mem_pass">
						
						<label for="usr">새 비밀번호</label>
						<input type="password" class="form-control pw" name="change_pass" id="pwd"> 
						<span class="sp"></span> <span id="idspan"></span>
						<div style='display: inline' class="msg"></div><br>
						
						<label for="usr">새 비밀번호 확인</label>
						<input type="password" class="form-control pw" name="new_pass" id="pwd2"><br>
						<span class="sp2"></span> <span id="idspan"></span>
						<div style='display: inline' class="msg2"></div>
						<br>
				...
				<form id="telUpdateForm" action="<%=request.getContextPath()%>/mypage/updateInfo.ddit">
					<div id="telUpdate" class="tab-pane fade">
						...
						<img src='./img/boy_basic.png'><br>
							...
							<span class="telmsgsp"></span> <span id="idspan"></span>
							<div style='display: inline' class="telmsg"></div>
						</div>
						<button type="submit" class="btn btn-success">수정하기</button>
						<button type="button" class="btn btn-default">취소</button>
					</div>
