...
public class InquiryListAction implements IAction {
	@Override
		...
		// 실제 처리할 내용은 이곳에 기술하고
		  ...
		  if(ccPage == null) ccPage = "1";
		   ...
		   IInquiryService service = InquiryServiceImpl.getService();
				...
				//List<freeBoardVO> boardList = service.selectAll();
				...
				// 페이징 처리
				 ...
				 // System.out.println(cPage);
				  ...
				  List<InquiryBoardVO> boardList = service.selectByPage(map);
						  
				 
				  
				  
				
				  req.setAttribute("inquiry", boardList);	// 자유게시판 데이터 값
				
				  req.setAttribute("tPage", totalPage); // 전체 페이지
				  req.setAttribute("sPage", startPage); // 시작 페이지 
				  req.setAttribute("ePage", endPage); // 마지막 페이지
				  	return "/inquiryboard/inquiryBoardList.jsp";
	}
