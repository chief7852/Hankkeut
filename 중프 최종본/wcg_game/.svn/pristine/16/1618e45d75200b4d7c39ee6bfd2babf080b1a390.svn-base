				...
				code += `
							<tr>
								...
								<td colspan="2">
									...
									<button type="button" onclick="refusal()">거절</button>
								</td>
							</tr>
						`;
			});
			$('#waitingF #wtbody').html(code);
		},
		error: function(xhr) {
			//alert("서버 상태 : " + xhr.status);
		},
			...
			$.each(res, function(i, v) {
				code += `
							<tr>
								<td>${v.friend_id}</td>
								<td>
									<button type="button" onclick="alert('준비중입니다');">귓속말</button>
								</td>
								<td>
									<button type="button" onclick="deleteFriend('${v.mem_id}','${v.friend_id}')">삭제</button>
								</td>
							</tr>
						`;
			});
...
deleteFriend = function(mem_id, friend_id) {
	$.ajax({
		url: "/wcggame/friendDelete.do",
		method: "post",
		data: {
			"mem_id" : mem_id,
			"friend_id" : friend_id
...
updateready1 = function(mem_id){
   var num = 1;
   $.ajax({
      
         url: "/wcggame/updateready.do",
         method : 'post',
         data : {
			"mem_id":mem_id,
			"num":  num},
...
updateready0 = function(mem_id){
   var num =0;
   $.ajax({
      
         url: "/wcggame/updateready.do",
         method : 'post',
         data : {"mem_id":mem_id,"num": num},
         success : function(res){
            alert("off로 적용되었습니다.")         },
         error : function(xhr){
